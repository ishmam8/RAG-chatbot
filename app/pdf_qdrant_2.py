...
from model_management import embeddings_model_large, llm_chat  # Ensure 'embeddings' and 'llm' are initialized
...
# COLLECTION_NAME = "buildsmart-pdf-collection_test"
...
async def fast_conversational_chat(user_input, retriever):
        """
        Optimized function to retrieve relevant documents and stream LLM response with Redis caching.
        """

        # Check cache first
        # cached_response = get_cached_response(user_input)
        # if cached_response:
        #     return cached_response  # Return cached response instantly
        ...
        response_stream = llm_chat.stream(f"Context: {context}\nUser: {user_input}\nBot:")

        response_placeholder = st.empty()  # Placeholder to update text dynamically
        full_response = ""

        for token in response_stream:
            full_response += str(token.content)  # Convert AIMessageChunk to string
            response_placeholder.text(full_response)  # Update response in real time
            time.sleep(0.005)  # Adjust delay for natural flow

        # Append metadata sources to the answer
        if metadata_info:
            full_response += f" (Sources: {'; '.join(metadata_info)})"

        # Store response in Redis cache
        # store_response(user_input, full_response)
...
def handle_pdf_3(uploaded_file):
    ...
    #         output = fast_conversational_chat(user_input, retriever)
    ...
    with container:
        ...
        if submit_button and user_input:
            # Ensure fast_conversational_chat is awaited if it's an async function
            if asyncio.iscoroutinefunction(fast_conversational_chat):
                output = asyncio.run(fast_conversational_chat(user_input, retriever))
            else:
                output = fast_conversational_chat(user_input, retriever)
    ...
    if st.session_state['generated']:
        with response_container:
            for i in range(len(st.session_state['generated'])):
                ...
                # Ensure message content is string (not coroutine)
                response = st.session_state["generated"][i]
                if isinstance(response, str):
                    message(response, key=str(i), avatar_style="bottts")
                else:
                    message(str(response), key=str(i), avatar_style="bottts")  # Convert non-string responses to string
    ...
    #     if submit_button and user_input:
    #         with response_container:
    #             message(user_input, is_user=True, avatar_style="fun-emoji")
    #
    #         response = fast_conversational_chat(user_input, retriever)
    #
    #         with response_container:
